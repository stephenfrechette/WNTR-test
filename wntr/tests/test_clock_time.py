...
class TestLoggerDictionaries(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        inp_file = resilienceMainDir + '/wntr/tests/networks_for_testing/test_clocktime.inp'
    ...
    def test_clock_time(self):
        ...
        self.clocktime_working = False
        link = self.wn.get_link('9')
        if 104400 in self._control_log.times[link]['status'] and 122400 in self._control_log.times[link]['status']:
            self.clocktime_working = True
        print self._control_log.times[link]
        self.assertEqual(self.clocktime_working, True)
